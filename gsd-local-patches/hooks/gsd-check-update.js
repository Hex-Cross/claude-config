...
// Clean up stale auto-trigger session dedup files (older than 24 hours)
const sessionsDir = path.join(globalConfigDir, 'hooks', 'sessions');
if (fs.existsSync(sessionsDir)) {
  try {
    const now = Date.now();
    const DAY_MS = 24 * 60 * 60 * 1000;
    for (const f of fs.readdirSync(sessionsDir)) {
      if (!f.startsWith('auto-trigger-') || !f.endsWith('.json')) continue;
      const fpath = path.join(sessionsDir, f);
      try {
        const stat = fs.statSync(fpath);
        if (now - stat.mtimeMs > DAY_MS) fs.unlinkSync(fpath);
